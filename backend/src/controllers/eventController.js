...
exports.getMyEvents = async (req, res) => {
  try {
    ...
    // This is called "Dynamic Query Building" - we only filter by provided params
    const whereClause = {
      OR: [
        ...
        { 
          participants: {
            some: { 
              userId: userId,
              status: { in: ['CONFIRMED', 'PENDING'] } // Only show if user is participating
            }
          }
        }
      ]
    ...
    // Date filtering - if specific date provided, show events for that day
    if (date) {
    ...
    // The "include" clause is Prisma's way of doing SQL JOINs
    const events = await prisma.event.findMany({
      ...
      include: {
        ...
        tags: true // Include event tags
      },
      orderBy: {
        startTime: 'asc' // Chronological order
      }
    });

    // Transform the data to match frontend expectations
    // This is called "Data Transformation Layer" - shaping DB data for the client
    const transformedEvents = events.map(event => ({
      ...
      // Assign a color based on event type (for UI consistency)
 ...
 * 
 * This uses more complex filtering logic:
 * - Events user hasn't joined yet
 * - Events matching user's skill level
 * - Events in user's area (optional)
 */
exports.getDiscoverEvents = async (req, res) => {
  try {
    ...
    const whereClause = {
      ...
      creatorId: { not: userId }, // Don't show user's own events
      // Exclude events user is already participating in
      NOT: {
        participants: {
          some: { userId: userId }
        }
    ...
    // Skill level filtering - show events within user's skill range
    if (skillLevel) {
      whereClause.OR = [
        { skillLevelMin: null, skillLevelMax: null }, // Open to all
        { 
    ...
    const transformedEvents = filteredEvents.map(event => ({
      ...
      entryFee: event.entryFee,
      rank: formatSkillLevel(event.skillLevelMin, event.skillLevelMax),
      tags: event.tags.map(tag => ({
        name: tag.tagName,
        color: tag.tagColor
      })),
 ...
 * Create a new event
 * 
 * This demonstrates a TRANSACTION - multiple database operations that
...
exports.createEvent = async (req, res) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Automatically add creator as a confirmed participant
    ...
    // Fetch the complete event data to return
  ...
  try {
    ...
    await prisma.eventParticipant.create({
      data: {
        ...
        userId: userId,
        status: 'CONFIRMED' // Could be PENDING if event requires approval
      }
    });
